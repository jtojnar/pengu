...
Polyline.prototype.getIntersections = function(line) {
	let lastCoords = this.points[0];
	let intersections = [];
	for (let coords of this.points) {
		let currentLine = new Line(lastCoords, coords);
		let inter = line.getIntersection(currentLine);
		if (inter !== false) {
...
Line.prototype.getIntersection = function(line2) {
	let line1 = this;
	let uline1_t = (line2.end.x - line2.start.x) * (line1.start.y - line2.start.y) - (line2.end.y - line2.start.y) * (line1.start.x - line2.start.x);
	let uline2_t = (line1.end.x - line1.start.x) * (line1.start.y - line2.start.y) - (line1.end.y - line1.start.y) * (line1.start.x - line2.start.x);
	let u_line2  = (line2.end.y - line2.start.y) * (line1.end.x - line1.start.x) - (line2.end.x - line2.start.x) * (line1.end.y - line1.start.y);

	if (u_line2 != 0) {
		let uline1 = uline1_t / u_line2;
		let uline2 = uline2_t / u_line2;
